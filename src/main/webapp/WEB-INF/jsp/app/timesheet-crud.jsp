		...
		<div id="search-panel-inner" data-spy="affix" data-offset-top="60">
			...
			<div class="search-form collapse">
				<form:form modelAttribute="searchCommand" id="timesheet-search" >
					...
					<tides:date_picker label = "label.timesheet.cutOffStart" path="cutOffStart" />
					<tides:date_picker label = "label.timesheet.cutOffEnd" path="cutOffEnd" />
					<hr/>
				...
				<table id="timesheet-results" class="footable table-bordered table-striped table-hover table-condensed" data-page="${results.currPage}" >
					<thead>
						<tr class="table-header">
							<!-- Define headers here -->
							<th><spring:message code="label.timesheet.cutOffStart"></spring:message></th>
							<th><spring:message code="label.timesheet.cutOffEnd"></spring:message></th>
					...
					<tbody>
						...
						<c:forEach items="${results.results}" var="record" varStatus="status">
							<tr data-id="${record.id}">
								...
								<td>
									<c:out value="${record.cutOffStart}"/>
								</td>
								<td>
									<c:out value="${record.cutOffEnd}"/>
								</td>
								<td>
									<c:out value="${record.employee.lastName}, ${record.employee.firstName}"/>
								</td>
								<td>
	...
	<div id="form-panel" >
		...
		<form:form modelAttribute="formCommand" id="timesheet-form">
			<div class="modal-body">
				...
				<tides:date_picker label = "label.timesheet.cutOffStart" path="cutOffStart" required="true" />
				<tides:date_picker label = "label.timesheet.cutOffEnd" path="cutOffEnd" required="true" />
				<tides:select label="label.timesheet.employee" path="employee" items="${employees}" itemLabel="lastName" itemValue="id" required="true"/>
